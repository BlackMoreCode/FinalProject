...
import {closeModal, openModal, setRejectModal} from "../../../context/redux/ModalReducer";
import Commons from "../../../util/Common";
import {ChangeWithSetter, KeyPress, SNSLoginType} from "../../../context/types";
import {Button,
  GoogleButton, InputField, KakaoButton, Line, LogoImg, ModalContent, NaverButton, SignupTextButton, Slash,
  SnsLoginText, SocialButtonsContainer, TextButton, TextButtonContainer} from "../Style";
...
const LoginModal = () => {
  ...
  const SNS_SIGN_IN_URL = (type : SNSLoginType) => `${Commons.BASE_URL}/api/v1/auth/oauth2/${type}`;
  ...
  const onClickLogin = async () => {
    try {
      ...
      if (loginRes !== null && loginRes.grantType === "Bearer") {
        ...
        dispatch(setRejectModal({ message: "ID와 PW가 다릅니다.", onCancel: () => {} }));
      }
    } catch (err: unknown) {
      if (axios.isAxiosError(err)) {
        ...
        if (err.response?.status === 405) {
          ...
          dispatch(setRejectModal({ message: "로그인에 실패 하였습니다.", onCancel: () => {} }));
        } else {
          console.log("서버와의 통신에 실패: ", err.message);
          dispatch(setRejectModal({ message: "서버와의 통신에 실패 하였습니다.", onCancel: () => {} }));
        }
      } else {
        console.log("예상치 못한 에러 발생: ", err);
        dispatch(setRejectModal({ message: "알 수 없는 오류가 발생하였습니다.", onCancel: () => {} }));
      }
    }
  };
  ...
  return (
    <Dialog open={loginModal} onClose={() => dispatch(closeModal("login"))} >
      ...
      <form onSubmit={(e) => e.preventDefault()}>
        ...
        <TextButtonContainer>
          <div>
            <TextButton onClick={() => dispatch(openModal("findId"))}>이메일 찾기</TextButton>
            <Slash>/</Slash>
            <TextButton onClick={() => dispatch(openModal("findPw"))}>비밀번호 찾기</TextButton>
          </div>
          <SignupTextButton onClick={() => dispatch(openModal("signup"))}>회원가입</SignupTextButton>
        </TextButtonContainer>
